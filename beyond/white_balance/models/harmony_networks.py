...
class HTTRDFCGenerator(nn.Module):
    def __init__(self, opt=None):
        super(HTTRDFCGenerator, self).__init__()
        dim = 256
        ...
        self.transformer_enc_AWB = transformer.TransformerDecoders(dim, nhead=opt.tr_r_dec_head, num_decoder_layers=opt.tr_r_dec_layers, dim_feedforward=dim*opt.dim_forward, activation=opt.tr_act)
        self.dec_AWB = nn.Sequential(
            nn.ConvTranspose2d(dim, opt.output_nc, kernel_size=opt.ksize, stride=opt.ksize, padding=0),
            nn.Tanh()
        )
        self.transformer_enc_T = transformer.TransformerDecoders(dim, nhead=opt.tr_r_dec_head, num_decoder_layers=opt.tr_r_dec_layers, dim_feedforward=dim*opt.dim_forward, activation=opt.tr_act)
        self.dec_T = nn.Sequential(
            nn.ConvTranspose2d(dim, opt.output_nc, kernel_size=opt.ksize, stride=opt.ksize, padding=0),
            nn.Tanh()
        )

        self.transformer_enc_S = transformer.TransformerDecoders(dim, nhead=opt.tr_r_dec_head, num_decoder_layers=opt.tr_r_dec_layers, dim_feedforward=dim*opt.dim_forward, activation=opt.tr_act)
    ...
    def forward(self, inputs, pixel_pos=None):
        ...
        AWS = self.rec(content, pixel_pos, self.transformer_enc_AWB, self.dec_AWB)
        T = self.rec(content, pixel_pos, self.transformer_enc_T, self.dec_T)
        S = self.rec(content, pixel_pos, self.transformer_enc_S, self.dec_S)
        return AWS, T, S
    def rec(self, content, pos, TR, DEC):
        L, bs, C  = content.size()
        content = TR(content, content, tgt_pos=pos, src_pos=pos)
        harmonized = DEC(content.permute(1,2,0).view(bs, C, int(math.sqrt(L)), int(math.sqrt(L))))
        return harmonized
