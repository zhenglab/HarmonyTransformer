...
class WBDataset(BaseDataset):
        ...
        if opt.isTrain==True:
            ...
            with open(self.trainfile,'r') as f:
                    for line in f.readlines():
                        ...
                        # self.imgfiles.append(line.rstrip().replace(".jpg", '.png'))
        elif opt.isTrain==False:
            # self.imgfiles = [join(opt.dataset_root, file) for file in listdir(opt.dataset_root)
            #             if file.endswith('.JPG') or file.endswith('.jpg')]
            # self.imgfiles = [join(opt.dataset_root, file) for file in listdir(opt.dataset_root)
            #             if file.endswith('.JPG') ]
            if self.opt.wb_action == 'wbc':
                self.imgfiles = [join(opt.dataset_root, file) for file in listdir(opt.dataset_root)
                            if file.endswith('.png') ]
            self.imgfiles.sort()
            # self.tatgetfiles = self.imgfiles
            # random.shuffle(self.tatgetfiles)
            # self.imgfiles = self.imgfiles[6500:]
            # self.imgfiles = []
            # fold = 1
            # files = loadmat(join('data/folds', 'fold%d_.mat' % fold))
    ...
    def __getitem__(self, index):
        if self.isTrain:
            ...
            img_file = self.imgfiles[index]
            if self.opt.wb_action == 'wbc':
                gt_path = img_file.replace('_input/','_gt/')
                tgt_path = self.imgfiles[-index-1].replace('_input/','_gt/')
            ...
            gt_image_resized = gt_image.resize([128,128])
            gt_image_resized = np.array(gt_image_resized)
            gt_img = gt_image_resized.transpose((2, 0, 1))
            gt_img = gt_img / 255
            gt_img = torch.from_numpy(gt_img)
            tgt_image = Image.open(tgt_path).convert('RGB')
            tgt_image_resized = tgt_image.resize([128,128])
            tgt_image_resized = np.array(tgt_image_resized)
            tgt_img = tgt_image_resized.transpose((2, 0, 1))
            tgt_img = tgt_img / 255
            tgt_img = torch.from_numpy(tgt_img)

            return {'image': img, 'gt_image': gt_img, 'tgt_image':tgt_img, 'img_path':img_file}
